function [Dxyn, varargout] = SPRING_PRAEGER_LIN_DISS(uxyn, pars, opt)
...
%derivatives since Kt is fixed when DFUN is set
    ...
    [Fxyn, dFxyndX] = SPRING_PRAEGER_LIN(uxyn, pars, opt);
    ...
    if nargout>=3
        ...
        varargout{2} = zeros(size(Dxyn,1), length(lpars)-1, 3);
        
        % JENKINS 4 - X
        varargout{2}(:, :, 1) = [opt.x.T{1}(:, 1).*(4.*ux-8.*Fs_x./Kt_x).*(Kt_x.*ux >= Fs_x), ... %Fs %opt.x.T{2}(:, 2).*(4.*Fs_x.^2./Kt_x.^2).*(Kt_x.*ux >= Fs_x), ... %Kt
        ...
        % JENKINS 4 - Y
        varargout{2}(:, :, 2) = [opt.y.T{1}(:, 1).*(4.*uy-8.*Fs_y./Kt_y).*(Kt_y.*uy >= Fs_y), ... %Fs % opt.y.T{2}(:, 2).*(4.*Fs_y.^2./Kt_y.^2).*(Kt_y.*uy >= Fs_y), ... %Kt
                                 zeros(size(uxyn(:, 1:2)))]; %derivative w.r.t. Kn (No Kt)
        ...
        varargout{2}(:, :, 3) = zeros(size(Dxyn,1), length(lpars)-1);
    end
end