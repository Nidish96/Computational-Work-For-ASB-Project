function [Dxyn, varargout] = SPRING_JENKINS_MOD_DISS(uxyn, pars, opt)
%JENKINS friction model for contact interface
% USAGE:
%  [Dxyn, varargout] = SPRING_JENKINS_MOD_DISS(uxyn, pars, opt);
% INPUTS:
...
%   varargout{2} - Jacobian w.r.t. parameters 
    ...
    %NOTE: Not technically correct, but this program is just reformatted
    %SPRING_JENKINS_DISS to give the correct number of zero colums for KP,
    ...
    %macroslip
    Dxyn(:, 1) = 4.*Fs_x.*(ux - Fs_x./Kt_x).*(Kt_x.*ux >= Fs_x);
    ...
    %Applying Masing, dissipation for a full loop. Only applies for
    %macroslip
    Dxyn(:, 2) = 4.*Fs_y.*(uy - Fs_y./Kt_y).*(Kt_y.*uy >= Fs_y);
    ...
    if nargout>=3
        ...
        % JENKINS 4 - X
        varargout{2}(:, :, 1) = [opt.x.T{1}(:, 1).*(4.*ux-8.*Fs_x./Kt_x).*(Kt_x.*ux >= Fs_x), ... %Fs
                                 opt.x.T{2}(:, 2).*(4.*Fs_x.^2./Kt_x.^2).*(Kt_x.*ux >= Fs_x), ... %Kt
        ...
        % JENKINS 4 - Y
        varargout{2}(:, :, 2) = [opt.y.T{1}(:, 1).*(4.*uy-8.*Fs_y./Kt_y).*(Kt_y.*uy >= Fs_y), ... %Fs
                                 opt.y.T{2}(:, 2).*(4.*Fs_y.^2./Kt_y.^2).*(Kt_y.*uy >= Fs_y), ... %Kt
                                 zeros(size(uxyn(:, 1:2)))]; %derivative w.r.t. Kp,Kn
