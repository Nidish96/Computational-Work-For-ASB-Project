function [Fxyn, varargout] = SPRING_BOUCWEN_N1(uxyn, pars, opt)
%BOUC-WEN friction model for contact interface
% USAGE:
%  [Fxyn, varargout] = SPRING_BOUCWEN_N1(uxyn, pars, opt);
% INPUTS:
...
%   varargout{2} - Jacobian w.r.t. parameters
    ...
    Fxyn(:, 1) = -sux.*A_x./eta_x .* (exp(-eta_x.*ux) - 1);
    ...
    %calculate the force values
    Fxyn(:, 2) = -suy.*A_y./eta_y .*(exp(-eta_y.*uy) - 1);
