function [Fxyn, varargout] = SPRING_IWAN4_MOD(uxyn, pars, opt)
    ...
    ux = sux.*uxyn(:,1);
	Fxyn(:,1) = ((Kt.*ux - CoefE.*ux.^(2+Chi)).*(ux<PhiMx) + (Fs).*(ux>=PhiMx)).*sux + Kp.*ux.*sux;
    ...
    uy = suy.*uxyn(:,2);
	Fxyn(:,2) = ((kt.*uy - coefE.*uy.^(2+chi)).*(uy<phiMx) + (fs).*(uy>=phiMx)).*suy + kp.*uy.*suy;
    ...
    %% Jacobians
    if nargout>=2
        varargout{1} = [(Kt-CoefE.*(2+Chi).*ux.^(1+Chi)).*(ux<PhiMx) + Kp, ...
            (kt-coefE.*(2+chi).*uy.^(1+chi)).*(uy<phiMx) + kp,...
            Kn];
    end
    if nargout>=3
        ...
        varargout{2}(:, end-1, 1) = opt.x.T{5}(:, 5).*uxyn(:, 1); %Kp
        ...
        varargout{2}(:, end-1, 2) = opt.y.T{5}(:, 5).*uxyn(:, 2); %Kp
